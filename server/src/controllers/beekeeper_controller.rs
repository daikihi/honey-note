...
pub async fn get_all_beekeepers() -> Result<actix_web::HttpResponse, actix_web::Error> {
    ...
    let results: Result<get_all_beekeepers_dto::GetAllBeekeepersResponseDto, AppError> =
        get_all_beekeepers::run(dto).await;
    let response: Result<actix_web::HttpResponse, actix_web::Error> =
        get_all_beekeepers_response_adapter(results);
    response
}
