...
async fn main_work() {
    let beekeeper_api_path: &str = "/honey-note/api/beekeepers";
    ...
    let result: Result<JsValue, JsValue> = get_list_data(beekeeper_api_path).await;
    match result {
        Ok(value) => {
...
fn write_beekeepers_to_table(beekeepers: Vec<ModelBeekeeper>, prefectures: Vec<ModelPrefecture>) {
    ...
    for beekeeper in beekeepers {
        ...
        row.set_inner_html(&format!(
            "<td>{}</td><td>{}</td><td>{}</td><td><a href='/honey_note/beekeepers/show.html?id={}'>詳細</a> | <a href='/honey_note/beekeepers/edit.html?id={}'>編集</a></td>",
            beekeeper_id,
            beekeeper.name_jp,
