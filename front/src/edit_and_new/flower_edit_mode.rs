...
use crate::edit_and_new::flower_new_mode::collect_form_data;
...
pub async fn setup_edit_mode(window: &Window, document: &Document) {
    ...
    let _ = fetch_and_prefill_flower(id, document).await;
    setup_save_handler(id, document);
}

async fn fetch_and_prefill_flower(id: i32, document: &Document) -> Result<(), JsValue> {
    let api_url = format!("/honey-note/api/flower/{}", id);
    let window = web_sys::window().unwrap();
    let resp_value = JsFuture::from(window.fetch_with_str(&api_url)).await?;
    let resp: Response = resp_value.dyn_into()?;
    
    if !resp.ok() {
        return Err(JsValue::from_str("Failed to fetch flower"));
    }

    let json = JsFuture::from(resp.json()?).await?;
    let flower: Flower = serde_wasm_bindgen::from_value(json).unwrap();

    set_input_value(document, "name_jp", &flower.name_jp);
    set_input_value(document, "name_en", &flower.name_en.unwrap_or_default());
    set_input_value(document, "scientific_name", &flower.scientific_name.unwrap_or_default());
    set_input_value(document, "short_note", &flower.short_note.unwrap_or_default());
    set_input_value(document, "flower_type", &flower.flower_type.unwrap_or_default());
    set_textarea_value(document, "note", &flower.note.unwrap_or_default());
...
async fn handle_update(id: i32, document: &Document) -> Result<(), JsValue> {
    let mut flower = collect_form_data(document)?;
    flower.id = Some(id as f64);
    let api_url = format!("/honey-note/api/flower/edit/{}", id);
